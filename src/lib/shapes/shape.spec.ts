...
describe('Common shape features', () => {
    ...
    test('intersecting a scaled shape with a ray', () => {
        ...
        expect(
            tuplesAreEqual(s.localRayFromBase!.origin, point(0, 0, -2.5))
        ).toBe(true);
        expect(
    ...
    test('intersecting a translated shape with a ray', () => {
        ...
        expect(s.localRayFromBase).not.toBeNull();
        expect(
            tuplesAreEqual(s.localRayFromBase!.origin, point(-5, 0, -5))
        ).toBe(true);
        expect(
            tuplesAreEqual(s.localRayFromBase!.direction, vector(0, 0, 1))
        ).toBe(true);
    });
    ...
    test('converting a normal from object to world space', () => {
        ...
        expect(tuplesAreEqual(n, vector(0.28571, 0.42857, -0.85714))).toBe(
            true
        );
    });
