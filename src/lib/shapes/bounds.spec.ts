...
import { Bounds, boundsContainsBounds, boundsContainsPoint, splitBounds } from "./bounds";
...
describe('Bounds', () => {
    ...
    each`
        ...
        ${point(8, 5, 3)}  | ${false}
        ${point(8, 1, 8)}  | ${false}
    `.test('checking to see if a box contains a given point', ({p, result}) => {
    ...
    `.test('checking to see if a box contains a given point', ({min, max, result}) => {
